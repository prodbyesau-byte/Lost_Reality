...
func _ready() -> void:
	...
	for id in StatCatalog.NAMES:
		...
		button.text = "+"
		button.tooltip_text = "Spend 1 attribute point on " + StatCatalog.NAMES[id]
...
func refresh() -> void:
	...
	for id in profession_labels:
		...
		profession_buttons[id].tooltip_text = "Requires Level %d" % maxi(1, int(definitions[id].unlock_requirements.minimum_level)) if not unlocked else "Activate this profession"
