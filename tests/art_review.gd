...
func run() -> void:
	...
	check(SaveConstants.VERSION == 3 and Progression.snapshot() == ProgressionSchema.defaults(), "Visual pass does not change save format or initial progression")
	...
	check(GameState.snapshot() == before, "Art-dressed scene transition preserves gameplay state")
